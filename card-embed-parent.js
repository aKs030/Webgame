...
(function () {
  ...
  function openModalForSrc(src, title) {
    ...
    overlay
      .querySelector(".card-embed-close")
  ...
  function applyAdaptiveModalSize(modal, width, height) {
    if (!modal) return;

    const targetWidth = clampSize(width + 32, 360, 1400);
    const targetHeight = clampSize(height + 84, 300, 980);

    modal.style.width = `min(95vw, ${targetWidth}px)`;
    modal.style.height = `min(94vh, ${targetHeight}px)`;
  }
...
`;
  ...
  function handleMessage(e) {
    const data = e.data || {};
    if (data && data.type === "card-fit-size") {
      const iframe = findIframeFromSource(e.source);
      if (!iframe) return;

      const modal = iframe.closest(".card-embed-modal");
      if (modal) {
        applyAdaptiveModalSize(modal, Number(data.width), Number(data.height));
      }
      return;
    }
